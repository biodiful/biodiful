...
describe('Survey Management Update Component', () => {
  ...
  let surveyFormService: SurveyFormService;
  ...
  beforeEach(() => {
    ...
    surveyFormService = TestBed.inject(SurveyFormService);
  ...
  describe('ngOnInit', () => {
    it('Should update editForm', () => {
      const survey: ISurvey = { id: 18583 };

      activatedRoute.data = of({ survey });
      comp.ngOnInit();

      expect(comp.survey).toEqual(survey);
    });
  });
  ...
  describe('save', () => {
    it('Should call update service on save for existing entity', () => {
      // GIVEN
      const saveSubject = new Subject<HttpResponse<ISurvey>>();
      const survey = { id: 18911 };
      jest.spyOn(surveyFormService, 'getSurvey').mockReturnValue(survey);
      ...
      // THEN
      expect(surveyFormService.getSurvey).toHaveBeenCalled();
      expect(comp.previousState).toHaveBeenCalled();
      expect(surveyService.update).toHaveBeenCalledWith(expect.objectContaining(survey));
    ...
    it('Should call create service on save for new entity', () => {
      ...
      const saveSubject = new Subject<HttpResponse<ISurvey>>();
      const survey = { id: 18911 };
      jest.spyOn(surveyFormService, 'getSurvey').mockReturnValue({ id: null });
      jest.spyOn(surveyService, 'create').mockReturnValue(saveSubject);
      jest.spyOn(comp, 'previousState');
      activatedRoute.data = of({ survey: null });
      comp.ngOnInit();
    ...
    it('Should set isSaving to false on error', () => {
      ...
      // THEN
      expect(surveyService.update).toHaveBeenCalled();
      expect(comp.isSaving).toEqual(false);
